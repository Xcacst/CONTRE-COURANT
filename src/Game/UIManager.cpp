...
UIManager::UIManager() {
  ...
  m_flashTimer = 0.0f;
  m_finSprite = App::CreateSprite("./data/TestData/Fin.bmp", 1, 1);
}
...
void UIManager::RenderVignette(Player *player, float currentDepth) {
  float factor = (currentDepth - GameConfig::START_DEPTH) /
                 (GameConfig::WIN_DEPTH - GameConfig::START_DEPTH);
  if (factor < 0.0f)
    factor = 0.0f;
  if (factor > 1.0f)
    factor = 1.0f;
  ...
  // Oscuridad total hacia el infinito (Triangle Strip) para tapar el resto de
  // la pantalla
...
void UIManager::TriggerFlash() { m_flashTimer = GameConfig::FLASH_DURATION; }
...
void UIManager::RenderFlash() {
  ...
  float alpha =
      (m_flashTimer / GameConfig::FLASH_DURATION) * GameConfig::FLASH_MAX_ALPHA;
...
void UIManager::RenderDepthBar(float currentDepth, float maxDepth) {
  ...
  float progress = (currentDepth - GameConfig::START_DEPTH) /
                   (maxDepth - GameConfig::START_DEPTH);
  if (progress < 0.0f)
    progress = 0.0f;
  if (progress > 1.0f)
    progress = 1.0f;

  // El relleno CAE desde arriba hacia abajo (más profundo = más relleno desde
  // arriba)
  ...
  for (float w = -4; w < barW + 4; ++w) {
    App::DrawLine(barX + w, indicatorY, barX + w, indicatorY + 2.0f, 1.0f, 1.0f,
                  1.0f);
  }

  // --- Etiquetas: BARCO arriba, META abajo ---
  App::Print((int)(barX - 14), (int)(topY + 10), "BOAT", 1.0f, 0.4f, 0.4f);
  App::Print((int)(barX - 10), (int)(bottomY - 18), "GOAL", 0.0f, 1.0f, 0.5f);
}
...
void UIManager::RenderHUD(Player *player, float currentDepth, float maxDepth) {
  ...
  for (const auto &popup : m_popups) {
    ...
    App::Print(popup.x, popup.y, popup.text.c_str(), popup.r * fade,
               popup.g * fade, popup.b * fade);
  }

  char buffer[128];

  int algaeCount = player->GetAlgaeCount();
  sprintf(buffer, "ALGAE: %d  (Press SPACE to call allies: %d)", algaeCount,
          GameConfig::ALGAE_COST_TO_CALL);
  App::Print(20, 755, buffer, 0.0f, 1.0f, 0.0f);
  ...
  App::Print(20, 735, buffer, swarmR, swarmG, swarmB);
  ...
  float effectiveSpeedX = GameConfig::PLAYER_SPEED_X *
                          (GameConfig::PLAYER_START_MASS / player->GetMass());
  ...
  App::Print(20, 700, buffer, 0.3f + 0.7f * agilityLoss,
             1.0f - 0.7f * agilityLoss, 0.0f);
  ...
  for (int line = 0; line < 5; ++line) {
    App::DrawLine(20, 715 - line, 20 + fillWidth, 715 - line, tensionFactor,
                  1.0f - tensionFactor, 0.0f);
  }
}
...
void UIManager::RenderMenu() {
  ...
  if (m_highScore > GameConfig::LOSE_DEPTH) {
    char buffer[128];
    sprintf(buffer, "Current Record: %.1f m", m_highScore / 10.0f);
    App::Print(320, 250, buffer, 0.0f, 1.0f, 1.0f);
  }
}

void UIManager::RenderWon() {
  if (m_finSprite) {
    m_finSprite->SetPosition(512.0f, 384.0f);
    m_finSprite->Draw();
  }

  // Si la imagen ya tiene la palabra "Fin" podemos mantener esto centrado o
  // quitarlo según prefieras
  App::Print(350, 400, "VICTORY!", 0.0f, 1.0f, 0.0f,
...
void UIManager::RenderLost() {
  App::Print(350, 400, "GAME OVER", 1.0f, 0.0f, 0.0f,
             GLUT_BITMAP_TIMES_ROMAN_24);
  App::Print(300, 350, "You were caught!", 1.0f, 1.0f, 1.0f);

  char buffer[128];
  sprintf(buffer, "You reached a depth of %.1f m", m_highScore / 10.0f);
  App::Print(280, 320, buffer, 1.0f, 1.0f, 0.0f);

  App::Print(320, 280, "Press R to Restart", 0.8f, 0.8f, 0.8f);
}

void UIManager::AddPopup(float x, float y, const char *text, float r, float g,
                         float b) {
  FloatingText ft;
  ft.x = x;
  ft.y = y;
...
float UIManager::GetHighScore() const { return m_highScore; }
